...
public class AccountController {
    ...
    public ResponseEntity<List<AvailableAccountResponse>> getAvailableAccounts(
        ...
        Long userId = 1L;
        return ResponseEntity.ok(accountService.getAvailableAccounts(userId));
    }
    ...
    /** 계좌 등록(연동) */
    @PostMapping
    public ResponseEntity<List<AccountResponse>> linkAccounts(
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(accountService.linkAccounts(userId, requestBody.getLinkableAccountIds()));
    }

    /** 주 계좌 변경 */
    @PatchMapping("/{accountId}/primary")
    public ResponseEntity<PrimaryAccountResponse> setPrimaryAccount(
            @PathVariable Long accountId,
            HttpServletRequest request
    ) {
        // JWT 토큰에서 userId 추출로 교체 필요(추후 삭제)
        Long userId = 1L;
        return ResponseEntity.ok(accountService.setPrimaryAccount(userId, accountId));
    }
    ...
    public ResponseEntity<DeleteAccountResponse> deleteAccount(
        ...
        // JWT 토큰에서 userId 추출로 교체 필요(추후 삭제)
        Long userId = 1L;
        return ResponseEntity.ok(accountService.deleteAccount(userId, accountId));
    }
}