...
public class CategoryController {
    ...
    @GetMapping
    public ResponseEntity<CategoryListResponseDTO> categoryListGet(
            @RequestParam("budgetType") BudgetType budgetType) {

        // JWT 토큰에서 userId 추출로 교체 필요(추후 삭제)
        Long userId = 1L;

        return ResponseEntity.ok(categoryService.getCategoryList(userId, budgetType));
    }

    // 2.2 카테고리 이름 변경
    @PatchMapping("/{categoryId}/label")
    public ResponseEntity<CategoryLabelResponseDTO> categoryLabelModify(
        ...
        return ResponseEntity.ok(categoryService.modifyCategoryLabel(userId, categoryId, dto));
    }
}
