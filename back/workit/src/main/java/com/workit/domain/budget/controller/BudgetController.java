...
public class BudgetController {
    ...
    @GetMapping
    public ResponseEntity<BudgetStatusResponseDTO> budgetStatusGet(
        ...
        return ResponseEntity.ok(budgetService.getBudgetStatus(userId, workationId, budgetType));
    }

    // 3.2 예산 카테고리별 세부 금액 설정
    // 새로 배분하는 것이므로 POST + 201. 이미 설정돼 있으면 409 로 수정 API 를 안내한다
    @PostMapping
    public ResponseEntity<BudgetSummaryResponseDTO> budgetSetupAdd(
            @PathVariable("workationId") Long workationId,
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(budgetService.setupBudget(userId, workationId, dto));
    }

    // 3.3 예산 세부 금액 수정
    // 해당 예산 유형의 배분 전체를 덮어쓰므로 PUT
    @PutMapping
    public ResponseEntity<BudgetSummaryResponseDTO> budgetModify(
            @PathVariable("workationId") Long workationId,
            @RequestBody BudgetSetupRequestDTO dto) {
        ...
        return ResponseEntity.ok(budgetService.modifyBudget(userId, workationId, dto));
    }

    // 3.4 예산 카테고리 추가 (＋ 버튼)
    // 예산 배분 행 하나를 새로 만드는 것이므로 하위 경로 /items 에 POST + 201
    @PostMapping("/items")
    public ResponseEntity<BudgetItemResponseDTO> budgetItemAdd(
            @PathVariable("workationId") Long workationId,
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(budgetService.addBudgetItem(userId, workationId, dto));
    }
