...
public class MerchantServiceImpl implements MerchantService {
    ...
    @Override
    @Transactional(readOnly = true)
    public PageResponseDTO<RestaurantListResponseDTO> findRestaurantList(
            RestaurantListRequestDTO request) {

        validateRestaurantListRequest(request);

        long totalElements = merchantMapper.countRestaurantList(
                request.getRegionId(),
                request.getFoodType(),
                request.getMinPrice(),
                request.getMaxPrice(),
        ...
        int offset = request.getPage() * request.getSize();
        List<RestaurantListResponseDTO> content = merchantMapper.selectRestaurantList(
                        request.getRegionId(),
                        request.getFoodType(),
                        request.getMinPrice(),
                        request.getMaxPrice(),
    ...
    @Override
    @Transactional(readOnly = true)
    public RestaurantDetailResponseDTO findRestaurantDetails(Long merchantId) {
        if (merchantId == null || merchantId < 1) {
            throw new IllegalArgumentException("가맹점 번호는 1 이상이어야 합니다.");
        }

        RestaurantDetailVO restaurant = merchantMapper.selectRestaurantDetails(merchantId);
        if (restaurant == null) {
            throw new BusinessException(MerchantErrorCode.RESTAURANT_NOT_FOUND);
        }

        return RestaurantDetailResponseDTO.from(
                restaurant,
                merchantMapper.selectMerchantTagNames(merchantId)
        );
    }
    ...
    // 필수 지역과 가격·평점·페이징 조건의 유효성 검증
    private void validateRestaurantListRequest(RestaurantListRequestDTO request) {
        if (request == null) {
            throw new IllegalArgumentException("조회 조건이 필요합니다.");
        }
