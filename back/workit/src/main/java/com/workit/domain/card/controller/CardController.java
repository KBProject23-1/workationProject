...
public class CardController {
    ...
    public ResponseEntity<List<AvailableCardResponse>> getAvailableCards(
            HttpServletRequest request
    ) {
        // JWT 토큰에서 userId 추출로 교체 필요(추후 삭제)
        Long userId = 1L;
        return ResponseEntity.ok(cardService.getAvailableCards(userId));
    }
    ...
    /** 카드 등록(연동) */
    @PostMapping
    public ResponseEntity<List<CardResponse>> linkCards(
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(cardService.linkCards(userId, requestBody.getLinkableCardIds()));
    }

    /** 대표 카드 설정 */
    @PatchMapping("/{cardsId}/primary")
    public ResponseEntity<PrimaryCardResponse> setPrimaryCard(
            @PathVariable Long cardsId,
            HttpServletRequest request
    ) {
        // JWT 토큰에서 userId 추출로 교체 필요(추후 삭제)
        Long userId = 1L;
        return ResponseEntity.ok(cardService.setPrimaryCard(userId, cardsId));
    }
    ...
    public ResponseEntity<NicknameUpdateResponse> updateNickname(
        ...
        // JWT 토큰에서 userId 추출로 교체 필요(추후 삭제)
        Long userId = 1L;
        return ResponseEntity.ok(cardService.updateNickname(userId, cardsId, requestBody.getCardNickname()));
    }
