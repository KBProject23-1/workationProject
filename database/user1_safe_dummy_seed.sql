-- 사용자 1 기준 핵심 시드 [안전 버전 — 로그인 계정을 덮어쓰지 않음]
-- 재실행 가능: 중복 시 기존 값은 덮어쓰기/유지하여 필요한 데이터만 정합
--
-- ★ 원본(user1_full_dummy_seed.sql)과의 차이:
--   users / user_auth INSERT 블록을 제거했다. 원본은 이 두 블록이
--   email_hash / phone_number_hash / password_hash / deleted_at 을 시드값으로
--   덮어써서, 회원가입한 계정의 로그인이 이후(재로그인 시) 깨졌다.
--   이 파일은 로그인 인증 관련 테이블을 건드리지 않으므로,
--   "회원가입 → 로그인 → 시드 실행 → 재로그인" 순서로 돌려도 로그인이 유지된다.
--   전제: user_id=1 인 계정이 이미 회원가입으로 존재해야 한다.
...
-- -----------------------------------------------------------------------------
-- user_profile / terms / user_terms_agreements
-- (★ users / user_auth 블록은 의도적으로 제거)
-- -----------------------------------------------------------------------------
-- [제거됨] users / user_auth INSERT ... ON DUPLICATE KEY UPDATE
--   원본은 여기서 users(email_hash / phone_number_hash / deleted_at) 와
--   user_auth(password_hash) 를 시드값으로 덮어써서, 회원가입한 계정의
--   로그인이 재로그인 시점에 깨졌다. 이 안전 버전은 로그인 인증 테이블을
--   건드리지 않는다. user_id=1 계정은 회원가입으로 이미 존재한다고 가정한다.
...
-- -----------------------------------------------------------------------------
-- tags
-- tags: name 에 UNIQUE 제약이 없어 단순 VALUES 는 재실행 시 중복 삽입된다.
-- NOT EXISTS 가드로 idempotent 하게 처리 (파일 상단 "재실행 가능" 원칙 유지)
INSERT INTO tags (name)
SELECT t.name
FROM (
    SELECT '조용함' AS name UNION ALL
    SELECT '해안'   UNION ALL
    SELECT '미식'   UNION ALL
    SELECT '액티비티' UNION ALL
    SELECT '워크숍' UNION ALL
    SELECT '바다뷰' UNION ALL
    SELECT '휴식'
) AS t
WHERE NOT EXISTS (SELECT 1 FROM tags x WHERE x.name = t.name);
...
VALUES
    (@WORKATION_ID, @TX_U1_WORK_ID, (SELECT id FROM expense_categories WHERE budget_type = 'WORK' AND code = 'RENT' LIMIT 1), (SELECT id FROM cards WHERE user_id = @USER_ID AND card_number = '4111111111111111' LIMIT 1), 'WORK', 120000.00, '해변 오피스 공유', '2026-08-12', '법인 공유오피스 좌석권 결제', 1, '2026-08-12 06:25:00', '2026-08-12 06:25:00'),
...
-- (주의: 위 SELECT 를 끝내는 세미콜론을 두면 ON DUPLICATE 절이 분리돼 구문 에러가 난다 — 원본 버그 수정)
ON DUPLICATE KEY UPDATE
